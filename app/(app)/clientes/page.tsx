    ...
    <div className="space-y-6">
      ...
      <Card>
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          {hasFilters && (
            <Link href="/clientes" className="text-sm text-ink-muted hover:text-ink-primary px-2 py-2.5">
              Limpar
            </Link>
          )}
        </form>
                ...
                <tr key={c.id} className="border-b border-border/60 last:border-0 hover:bg-surface-raised/40">
                  <td className="py-3 px-5">
                    <div className="flex items-center gap-1.5">
                      <Link href={`/clientes/${c.id}`} className="text-ink-primary font-medium hover:text-gold-400">
                        {c.name}
                      </Link>
                      {birthdaySoon && (
                        <span
