    ...
    <div className="space-y-6">
      ...
      <Card>
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          {statusFilter && (
            <Link href="/leads" className="text-sm text-ink-muted hover:text-ink-primary px-2 py-2.5">
              Limpar
            </Link>
          )}
        </form>
                ...
                <tr key={lead.id} className="border-b border-border/60 last:border-0 hover:bg-surface-raised/40">
                  ...
                  <td className="py-3 px-4">
                    {lead.status === "CONVERTIDO" && lead.convertedCustomer ? (
                      <Link href={`/clientes/${lead.convertedCustomer.id}`} className="text-xs text-gold-400 hover:text-gold-300">
                        Ver cliente
                      </Link>
                    ) : lead.status === "PERDIDO" ? (
