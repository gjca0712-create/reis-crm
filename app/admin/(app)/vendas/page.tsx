  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <LinkButton href="/admin/vendas/novo">
      ...
      <Card>
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          {entregaFilter && (
            <Link href="/admin/vendas" className="text-sm text-ink-muted hover:text-ink-primary px-2 py-2.5">
              Limpar
            </Link>
          )}
        </form>
              ...
              {sales.map((s) => (
                <tr key={s.id} className="border-b border-border/60 last:border-0 hover:bg-surface-raised/40">
                  ...
                  <td className="py-3 px-4">
                    <Link href={`/admin/clientes/${s.customer.id}`} className="text-ink-primary font-medium hover:text-gold-400">
                      {s.customer.name}
                    </Link>
                  </td>
                  ...
                  <td className="py-3 px-4 text-ink-secondary">
                    {s.partner ? (
                      <Link href={`/admin/indicadores/${s.partner.id}`} className="hover:text-gold-400">
                        {s.partner.name}
                      </Link>
                    ) : (
                      "—"
