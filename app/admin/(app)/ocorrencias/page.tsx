  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <LinkButton href="/admin/ocorrencias/novo">
      ...
      <Card>
        <form method="get" className="flex flex-wrap items-end gap-3">
          ...
          {tipoFilter && (
            <Link href={`/admin/ocorrencias?mes=${mes}`} className="text-sm text-ink-muted hover:text-ink-primary px-2 py-2.5">
              Limpar tipo
            </Link>
          )}
        </form>
              ...
              {rows.map((o) => (
                <tr key={o.id} className="border-b border-border/60 last:border-0 hover:bg-surface-raised/40">
                  ...
                  <td className="py-3 px-4 text-ink-secondary">
                    {o.customer ? (
                      <Link href={`/admin/clientes/${o.customer.id}`} className="hover:text-gold-400">
                        {o.customer.name}
                      </Link>
                    ) : (
                      "—"
